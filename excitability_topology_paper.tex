...
If $\Excit(i,j)$ is directly observable by all agents for all $(i,j) \in E$, then for any modulation operator $\Mod$ and any token $\tau \in \Theta$, the information conveyed by $\Mod(\tau, i)$ about $i$'s subsequent coordination behavior is a subset of the information already available in~$\Excit$.
\end{proposition}
...
Curvature-weighted allocation (\Cref{def:allocation}) has computational complexity $O(|E|)$ for a single allocation round, where $|E|$ is the edge count. By contrast, Nash equilibrium computation is PPAD-complete~\cite{daskalakis2009}, and Arrow-Debreu market equilibrium faces similar complexity barriers~\cite{chen2009arrow}.
\end{proposition}
