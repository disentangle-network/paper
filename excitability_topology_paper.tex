...
\newcommand{\topomass}{M}
\newcommand{\excit}{\varepsilon}
\newcommand{\Excit}{\mathcal{E}}
...
\author{
\IEEEauthorblockN{Larsen Close}
\IEEEauthorblockA{Independent Researcher\\
\texttt{larsen@disentangle.network}}
}
...
% ════════════════════════════════════════════════════════════════
\begin{abstract}
Currency is a technology for remotely modulating excitability topologies across agents whose internal states are opaque.
Price signals are a lossy proxy for excitability gradients---they encode ``what will shift your action-readiness'' into a transferable scalar.
A system that makes excitability topology directly legible---via geometric measurement of coordination quality (curvature) and coordination formation rate (curvature derivative)---renders the proxy unnecessary.
We formalize this claim information-theoretically, showing that under complete excitability legibility the mutual information between price and excitability vanishes.
We demonstrate the claim mechanistically via the Disentangle protocol's Jaccard-based Ollivier-Ricci curvature, mapping protocol primitives to a formal excitability framework.
The curvature derivative $\partial\kJ/\partial d$ serves as a dopaminergic market signal: agents interpreting it as ``return on investment'' and agents interpreting it as ``where is genuine information being created'' produce identical graph dynamics, making the transition from price-mediated to topology-mediated coordination structurally gradual and ideology-independent.
\end{abstract}

\begin{IEEEkeywords}
excitability topology, curvature, Ollivier-Ricci, consensus, resource allocation, dopamine, price theory, information theory
\end{IEEEkeywords}
...
The information that price signals encode is already available in the topology.
...
Let $\mathcal{A}$ be a finite set of agents and $G = (\mathcal{A}, E)$ an interaction graph. An \emph{excitability function} is a map
\begin{equation}
...
The \emph{excitability topology} $\Topo_\Excit$ on $(\mathcal{A}, E)$ is the weighted graph where each edge $(i,j) \in E$ carries weight $\Excit(i,j) \cdot \Excit(j,i)$, representing mutual coordination readiness.
\end{definition}
...
\begin{equation}
  \Mod : \mathcal{T} \times \mathcal{A} \to \Delta\Excit
\end{equation}
where $\mathcal{T}$ is a set of transferable tokens and $\Delta\Excit$ denotes a change in the excitability function. For token $\tau \in \mathcal{T}$ and agent $i \in \mathcal{A}$, $\Mod(\tau, i)$ is the shift in $i$'s excitability landscape induced by receiving $\tau$.
\end{definition}
...
If $\Excit(i,j)$ is directly observable by all agents for all $(i,j) \in E$, then for any modulation operator $\Mod$ and any token $\tau$, the information conveyed by $\Mod(\tau, i)$ about $i$'s subsequent coordination behavior is a subset of the information already available in $\Excit$.
\end{proposition}
...
\begin{equation}\label{eq:redundancy}
  \Info(P\,;\,\Excit \mid \Topo) \leq \Ent(\Excit) - \Ent(\Excit \mid \Topo)
\end{equation}
...
\begin{proof}
By the data processing inequality, $\Info(P\,;\,\Excit \mid \Topo) \leq \Info(\Excit\,;\,\Excit \mid \Topo) = \Ent(\Excit \mid \Topo)$.
If $\Topo$ resolves $\Excit$ completely, $\Ent(\Excit \mid \Topo) = 0$, so $\Info(P\,;\,\Excit \mid \Topo) = 0$.
\end{proof}

\begin{remark}
\Cref{thm:redundancy} does not claim that price becomes meaningless. It claims that price becomes \emph{redundant}: all information it encodes is already available through the topology. In practice, partial legibility yields partial redundancy, and the transition is continuous.
\end{remark}
...
The mapping is structural, not metaphorical. Both systems---neural circuits and curvature-measured interaction graphs---implement the same computational primitive: they measure the quality and rate-of-change of coordination across a topology, and they gate participation based on accumulated coordination history.
...
\begin{scope}[shift={(4.5,0)}]
  ...
  \node[annot] at (0, -2.8) {Green: high curvature\\(measured from topology)\\Threshold: CoherenceMin};
\end{scope}

\end{tikzpicture}
\caption{Structural analogy between neural excitability topology (left) and graph curvature topology (right). In both systems, the \emph{rate of change} of the topology (dopamine signal / curvature derivative) carries the actionable information, while accumulated history (LTP / topological mass) gates participation. Red/dark green nodes indicate high excitability or curvature.}
...
For edge $(i,j)$ in a DAG with topological depth function $d(\cdot)$, the curvature derivative is
\begin{equation}\label{eq:curv-deriv}
  \frac{\partial\kJ(i,j)}{\partial d} = \lim_{\Delta d \to 1} \frac{\kJ^{(d+\Delta d)}(i,j) - \kJ^{(d)}(i,j)}{\Delta d}
\end{equation}
where $\kJ^{(d)}$ denotes the curvature computed from the subgraph at depth $d$.
\end{definition}
...
The following parallel is exact:
\begin{itemize}
...
\begin{equation}\label{eq:allocation}
  a(i) = R \cdot \frac{\max\bigl(0,\; \nabla\kJ(i)\bigr)}{\sum_{k \in \mathcal{A}_{\text{elig}}} \max\bigl(0,\; \nabla\kJ(k)\bigr)}
\end{equation}
where $\nabla\kJ(i) = \frac{1}{|N(i)|}\sum_{j \in N(i)} \partial\kJ(i,j)/\partial d$ is the mean curvature derivative at $i$.
\end{definition}
...
\begin{itemize}
  ...
  \item Agents with declining coordination quality ($\nabla\kJ < 0$) receive zero allocation---the gradient automatically redirects resources to where coherence is actively forming.
  \item Sybil agents, whose boundary edges exhibit negative curvature~\cite{close2026}, have low or negative $\nabla\kJ$ and are excluded from allocation.
\end{itemize}

\begin{proposition}[Allocation Overhead]\label{prop:overhead}
Curvature-weighted allocation (\Cref{def:allocation}) has computational complexity $O(|E|)$ for a single allocation round, where $|E|$ is the edge count. Walrasian equilibrium computation is PPAD-complete in general~\cite{daskalakis2009}.
\end{proposition}
...
It is closer to a \emph{topological game}~\cite{bruin2013}, where the strategy space itself has geometric structure, and the topology constrains which games agents can enter.

\begin{theorem}[Non-Applicability of Classical Nash Equilibrium]\label{thm:nash}
In a curvature-gated coordination system $\Gamma_\kappa = (\mathcal{A}, \{S_i(\kJ)\}, \{u_i(\kJ)\})$ where both action sets and utility functions depend on the curvature field $\kJ$, and $\kJ$ is a function of the joint strategy profile, the fixed-point conditions for Nash equilibrium
\begin{equation}
  \forall i,\; s_i^* \in \arg\max_{s_i \in S_i} u_i(s_i, s_{-i}^*)
\end{equation}
are not well-defined, because $S_i$ and $u_i$ are themselves functions of $s^*$ through $\kJ(s^*)$.
\end{theorem}
...
The relevant equilibrium concept is instead a \emph{geometric attractor}:
...
Informally: ``grow local curvature or become isolated'' is a geometric attractor, not a Nash equilibrium.
The dynamics are convergent because curvature throttling~\cite{close2026} makes defection self-punishing---the act of fabricating coordination (Sybil behavior) creates the negative curvature that reduces the defector's influence.
...
\end{tikzpicture}
\caption{Curvature derivative field over an example interaction graph. Node color indicates $\partial\kJ/\partial d$ (red: decreasing coordination quality; green: increasing). Arrows show the gradient direction and magnitude. Resources flow rightward---toward the region of active coherence formation---regardless of whether agents interpret the gradient as ``ROI'' or ``information creation.''}
...
\begin{itemize}
  \item \textbf{Agent $A$ (pre-shift)}: interprets $\nabla\kJ > 0$ as ``high ROI region.'' Follows the gradient to maximize returns.
  \item \textbf{Agent $B$ (post-shift)}: interprets $\nabla\kJ > 0$ as ``region of genuine information creation.'' Follows the gradient to maximize coherence contribution.
\end{itemize}
...
Let $\sigma_A$ and $\sigma_B$ be the strategies of agents $A$ and $B$ where $\sigma_A$ maximizes a utility function $u_A(\nabla\kJ)$ and $\sigma_B$ maximizes $u_B(\nabla\kJ)$, with $u_A \neq u_B$ but both monotonically increasing in $\nabla\kJ$. Then the graph evolution $G(t+1)$ is identical under $\sigma_A$ and $\sigma_B$:
\begin{equation}
  G^{(\sigma_A)}(t+1) = G^{(\sigma_B)}(t+1)
\end{equation}
whenever $\arg\max_{j \in N(i)} \nabla\kJ(i,j)$ is unique.
\end{proposition}

\begin{proof}
Both strategies select the neighbor $j^* = \arg\max_{j \in N(i)} \nabla\kJ(i,j)$ for the next coordination action. Since the gradient is a property of the graph (not of the agent's interpretation), and both utilities are monotone in $\nabla\kJ$, both agents take the same action. The resulting graph modification is therefore identical.
\end{proof}
...
% The shared gradient
\node[rectangle, draw=blue!70, fill=blue!10, rounded corners=4pt, minimum width=50mm, minimum height=10mm, font=\small\bfseries] (grad) at (0, 0) {$\nabla\kJ > 0$ (same signal)};
...
\end{tikzpicture}
\caption{The transition path: two interpretive frameworks, one gradient, identical behavior. Pre-shift agents follow $\nabla\kJ$ for ROI; post-shift agents follow it for coherence. The graph cannot distinguish between them.}
...
\begin{proposition}[Modulation Resistance of Self-Governed Agents]\label{prop:self-gov}
For a self-governed agent $i$ (\Cref{def:self-gov}), $\Mod(\tau, i) = 0$ for all tokens $\tau$. The only path to coordination with $i$ is through achieving $\kJ(i,j) > 0$ via genuine structural integration.
\end{proposition}
...
In the Disentangle protocol, if agent $i$ receives resources via Commons Pool allocation (\Cref{def:allocation}) but fails to produce coherent coordination (i.e., $\partial\kJ(i,j)/\partial d \leq 0$ for subsequent interactions), then $i$'s allocation in future rounds decreases to zero and $i$'s topological mass decays exponentially. No external enforcement is required.
\end{proposition}

\begin{proof}
By \Cref{def:allocation}, allocation is proportional to $\max(0, \nabla\kJ(i))$. If $\nabla\kJ(i) \leq 0$, allocation is zero. By the mass decay property~\cite{close2026}, $\topomass_{\text{decayed}}(i, t) = \topomass(i) \cdot 2^{-(t - t_{\text{last}})/T_{1/2}}$, so without coherent participation, mass decays with half-life $T_{1/2}$. The consequence chain closes: non-performance $\to$ curvature decline $\to$ allocation loss $\to$ mass decay.
\end{proof}
...
Consider a Commons Pool containing 1000 units of compute, with four eligible agents:
...
However, maintaining the full curvature derivative field over a large graph requires $O(|E|)$ computation per epoch.
...
Bootstrap mechanisms are required for new agents entering the network.

\textbf{Partial legibility.}
...
The transition is therefore asymptotic, not binary.
...
\textbf{Adversarial gradient manipulation.}
Can an agent manipulate $\nabla\kJ$ in their favor without genuine coordination?
...
Independent Researcher, Feb.\ 2026.
...
\bibitem{bruin2013}
H.~Bruin and S.~Kolyada,
``Topological games and their applications to dynamical systems,''
\emph{Topology and its Applications}, vol.~160, no.~15, pp.~2006--2014, 2013.
